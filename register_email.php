...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'send_otp') {
        ...
        if (empty($email)) {
            ...
            $result = createEmailOTPGmail($email);
    ...
    elseif (isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        if (empty($firstName) || empty($lastName) || empty($username) || empty($password)) {
        ...
        } elseif (!isset($_SESSION['email_verified']) || !$_SESSION['email_verified']) {
            $error = 'Devi verificare l\'email';
        } else {
            ...
            if ($result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("
                    INSERT INTO users (username, first_name, last_name, email, password, phone_verified, full_name) 
                    VALUES (?, ?, ?, ?, ?, 1, ?)
                ");
                $fullName = $firstName . ' ' . $lastName;
                $stmt->bind_param("ssssss", $username, $firstName, $lastName, $email, $hashedPassword, $fullName);
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" 
                                       ...
                                       required
                                       autofocus>
                                <small class="text-muted">Riceverai il codice OTP a questa email</small>
                            </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" 
                                       ...
                                       required
                                       autofocus>
                                <small class="text-muted">Controlla la email <?php echo htmlspecialchars($_SESSION['registration_email'] ?? ''); ?></small>
                            </div>
